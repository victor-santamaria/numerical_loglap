...
%DIF DEL 1.tex   Tue Nov 21 18:45:29 2023
%DIF ADD 2.tex   Sun Aug 11 11:16:42 2024
...
CONAHCYT \DIFdelbegin \DIFdel{grant }\DIFdelend \DIFaddbegin \DIFadd{grants }\DIFaddend A1-S-10457 \DIFaddbegin \DIFadd{and  CBF2023-2024-116 }\DIFaddend (Mexico) and by UNAM-DGAPA-PAPIIT grant IA100923 (Mexico).}\and
Leonard Sinsch
}
...
%DIFDELCMD < We present the numerical analysis of a finite element method (FEM) for one-dimensional Dirichlet problems involving the logarithmic Laplacian (the pseudo-differential operator that appears as a first-order expansion of the fractional Laplacian as the exponent $s\to 0^+$). Our analysis exhibits new phenomena in this setting; in particular, using recently obtained regularity results, we prove rigorous error estimates and provide a logarithmic order of convergence in the energy norm using  suitable \emph{log}-weighted spaces.
%DIFDELCMD < Numerical evidence suggests that this type of rate cannot be improved.
%DIFDELCMD < Moreover, we show that the stiffness matrix of logarithmic problems can be obtained as the derivative of the fractional stiffness matrix evaluated at $s=0$.
%DIFDELCMD < Lastly, we investigate the relationship between the discrete eigenvalue problem and its convergence to the continuous one.
%DIFDELCMD < }
%DIFDELCMD < %%%
...
In this paper we analyze \DIFdelbegin \DIFdel{the implementation of }\DIFdelend \DIFaddbegin \DIFadd{and implement }\DIFaddend the finite element method (FEM) to one-dimensional nonlocal Dirichlet boundary value problems involving the logarithmic Laplacian such as
...
\begin{enumerate}
    ...
    \item The operator $L_\Delta$ does not satisfy the maximum principle in general. Nevertheless, positivity preserving properties do hold under additional \DIFdelbegin \DIFdel{geometric restrictions on the }\DIFdelend \DIFaddbegin \DIFadd{assumptions on the size of the }\DIFaddend domain, see \cite[Theorem 1.8 and Corollary 1.9]{CW19}.
...
Our main result (see Theorem~\ref{main:thm:intro} below) states that the FEM numerical approximation of the weak solution converges logarithmically; in particular, we show that the norm of the numerical error can be bounded in terms of $|\ln(h)|^{-\alpha}$, where $h$ is the discretization parameter and $\alpha\in(0,1)$. We also provide numerical evidence that suggests that this kind of logarithmic bounds \DIFdelbegin \DIFdel{are }\DIFdelend \DIFaddbegin \DIFadd{could be }\DIFaddend optimal, in the sense that no polynomial bound of the type $h^\beta$ for some $\beta>0$ can be expected in general in this setting.  This contrasts with the convergence rate $h^\frac{1}{2}|\ln h|$ observed for the fractional Laplacian for $s\in(0,\frac{1}{2})$ (see \cite[Theorem 3.31]{Bor17}) and the well-known rate $h^2$ for the standard Laplacian.
...
\mathbb H(\Omega)\hookrightarrow L^2(\Omega)\qquad \DIFdelbegin \DIFdel{\text{ is compact.}
}\DIFdelend \DIFaddbegin \DIFadd{\text{ is compact,}
}\DIFaddend \end{align}
\DIFaddbegin \DIFadd{see \mbox{%DIFAUXCMD
\cite[Theorem 2.1]{CdP18}}\hspace{0pt}%DIFAUXCMD
. }\DIFaddend Note that the space $\mH(\Omega)$ imposes very little regularity restrictions on its elements, and in particular step functions belong to $\mH(\Omega)$. As a consequence, this space does not allow a notion of trace for its elements.
...
\DIFdelbegin \DIFdel{To implement the FEM in logarithmic problems , the regularity theory }\DIFdelend \DIFaddbegin \DIFadd{The analysis of FEM in nonlocal problems relies strongly on fine regularity properties }\DIFaddend of weak solutions\DIFdelbegin \DIFdel{of }%DIFDELCMD < \eqref{P:intro} %%%
\DIFdel{is a key element}\DIFdelend . This kind of results face important challenges \DIFdelbegin \DIFdel{, due to the lack of good scaling properties of the operator $L_\Delta$}\DIFdelend \DIFaddbegin \DIFadd{in the logarithmic setting, where this theory is still underdeveloped}\DIFaddend ; however, recently in \cite{CS22} the regularity of weak solutions in log-Hölder spaces has been studied.  Since these results play a fundamental role in our approach (in particular the estimate \eqref{eq:log_problem} below), it is worth to describe them in detail. Let $\ell:(0,\infty)\to (0,1]$ be the modulus of continuity given by
...
The next Fredholm-alternative-type result characterizes the existence \DIFaddbegin \DIFadd{and regularity }\DIFaddend of classical solutions of \eqref{P:intro}.
...
\begin{theorem}[Theorem 1.1 in \cite{CS22}]\label{eq:regularity}
    ...
    \begin{enumerate}[label=\roman*)]
        ...
        %
        \item There is a non-trivial classical solution \DIFdelbegin \DIFdel{$u\in X^{\alpha}(\Omega)$ }\DIFdelend \DIFaddbegin \DIFadd{$u\in {\mathcal X}^{\alpha}(\Omega)$ }\DIFaddend of $L_{\Delta} u=0$.
    \end{enumerate}
\end{theorem}
...
%
\begin{equation*}
    \mathcal V_h=\{v\in \mathcal C^0(\Omega): v=0 \textnormal{ in } \R\setminus \Omega, \;\; v|_{\Omega_i}\in \mathcal P_1, \;\;  i=0,\ldots,N\}\DIFdelbegin \DIFdel{.
...
\end{equation}
 ...
 Due to the nonlocal nature of $L_\Delta$, we cannot use the standard theory of FEM (note that the norms are not additive with respect to disjoint domains). Instead, for the proof of Theorem~\ref{main:thm:intro}, we \DIFdelbegin \DIFdel{follow }\DIFdelend \DIFaddbegin \DIFadd{extend }\DIFaddend the general strategy implemented in \cite{Bor17}, where the FEM is \DIFaddbegin \DIFadd{analyzed and }\DIFaddend used to approximate solutions of the fractional Laplacian\DIFdelbegin \DIFdel{and to show the convergence of the method}\DIFdelend .  The overall scheme is as follows: If $u$ is a weak solution that we wish to approximate with \DIFdelbegin \DIFdel{a finite element approximation}\DIFdelend \DIFaddbegin \DIFadd{finite elements}\DIFaddend , one defines first an \emph{interpolator} denoted by $I_h u$.  Then, via a localization of the norms (see Lemma~\ref{lem:localization_enorm}) one can bound the global interpolation error $\|u-I_h u\|_{\mathbb H(\Omega)}$ (see Proposition~\ref{eq:estab_H_c_infty}) in terms of a Hilbert space with logarithmic weights (see \eqref{H:weights:def}).  Then, with a Cea's-Lemma-type result (Lemma~\ref{lem:cea_mg}), we can bound the norm $\|u-u_h\|_{\mathbb H(\Omega)}$ in terms of $\|u-I_h u\|_{\mathbb H(\Omega)}$.  Finally, the regularity estimate \eqref{eq:regularity_classical} comes into play to obtain the final estimate stated in Theorem~\ref{main:thm:intro}.
 ...
 \DIFdelbegin \DIFdel{For the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend algorithmic implementation of the FEM \DIFdelbegin \DIFdel{, we also present }\DIFdelend \DIFaddbegin \DIFadd{can be done, as usual, by computing the entries of the corresponding stiffness matrix numerically. However, here we present an alternative, which we believe that also gives }\DIFaddend a new perspective \DIFaddbegin \DIFadd{on how to understand the expansion }\eqref{intro:exp} \DIFadd{from the FEM point of view. Let us be more precise}\DIFaddend . Typically, in order to obtain the finite element approximation $u_h$ given by \eqref{eq:discr_problem:intro}, \DIFdelbegin \DIFdel{one needs to compute the so-called }\emph{\DIFdel{stiffness matrix}}%DIFAUXCMD
\DIFdel{. For this, }\DIFdelend a finite element basis $(\varphi_i)_{i=0}^{N+1}$ of $\mathcal V_h$ is considered (see \eqref{eq:def_basis}-\eqref{eq:def_basis_ext}) and a matrix with entries
 \begin{align}\label{s:m:intro}
 \mathcal A_{h}^{L}=(\cE_L(\varphi_i,\varphi_j))_{i,j=0}^N
 \end{align}
 is calculated.  \DIFaddbegin \DIFadd{The matrix $\mathcal A_{h}^{L}$ is called the stiffness matrix for the logarithmic Laplacian.  }\DIFaddend Then, solving the linear algebraic problem
 \begin{align*}
...
\end{equation*}
This can be seen in relation to the numerical approximation of the eigenvalues $\lambda_{k}^s$, $k\in\N$, $s\in(0,1)$ of the fractional Laplacian $(-\Delta)^s$ in $(-1,1)$ given in \cite{K12} in combination \DIFdelbegin \DIFdel{to }\DIFdelend \DIFaddbegin \DIFadd{with }\DIFaddend the fact that  \DIFdelbegin \DIFdel{it holds }\DIFdelend $\lambda_k^s=1+s\lambda_k+o(s)$ as $s\to 0^+$ for any $k\in\N$, see \cite{FJW22}. Indeed, from \cite[Table 1]{K12} (using only the numerical approximations) we have the following.
\begin{center}
...
\begin{figure}[htb]
	...
	legend plot pos=left, xtick={0,0.1,0.2}, xticklabels={-0.1,0,0.1} , ymin=-0.01,
	legend style={cells={anchor=west},draw=none}]
	%	    
	\pgfplotstableread{sol-log_20-06-2023_12h20.org}\solution      
	...
	legend plot pos=left, xtick={0,1,2}, xticklabels={-1,0,1}, ymax=0.01,
	legend style={cells={anchor=west},draw=none}]
	%	    
	\pgfplotstableread{sol-log_20-06-2023_12h25.org}\solution      
	...
	legend plot pos=left, xtick={0,8,16}, xticklabels={-8,0,8},
	legend style={cells={anchor=west},draw=none}]
	%	    
	\pgfplotstableread{sol-log_20-06-2023_17h12.org}\solution      
...
The paper is organized as follows. In \DIFdelbegin \DIFdel{Section~\ref{sec:localization-norm} }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\Cref{sec:three}}\hspace{0pt}%DIFAUXCMD
, }\DIFaddend we present the localization of the $\mathbb{H}$-norm \DIFdelbegin \DIFdel{. The main goal in this section is to provide the convergence of the method, see Theorem~\ref{density:thm} and some interpolation-type estimates}\DIFdelend \DIFaddbegin \DIFadd{and some useful approximation properties and results}\DIFaddend . In Section~\ref{sec:error} we give the proof of Theorem~\ref{main:thm:intro}. We calculate explicitly the stiffness matrix for the logarithmic Laplacian in Section~\ref{sec:stiffness} and we discuss the optimality of the convergence rate in Section~\ref{sec:numerics}. Finally, in Section~\ref{eigenvalue approximation:sec}, we give the proof of Proposition~\ref{eigenvalue approximation} and show that the eigenvalues of the stiffness matrix converge to \DIFaddbegin \DIFadd{the }\DIFaddend eigenvalues of $L_{\Delta}$ as the discretization parameter tends to zero, see Proposition~\ref{prop:eigen-approx}.
...
%
\begin{align}
    \weH{\beta}&:=\left\{ u\in L^2(\Omega): \int_{\Omega}\int_{\Omega}\frac{|u(x)-u(y)|^2}{|x-y|\ell(|x-y|)^{\beta}}dydx<+\infty \quad\textnormal{ and }\quad  \int_{\Omega}|u(x)|^2\ell^{-\beta}(\DIFdelbegin \DIFdel{\delta}\DIFdelend \DIFaddbegin \DIFadd{d}\DIFaddend (x))dx <+\infty\right\},\label{H:weights:def}
\end{align}
where \DIFdelbegin \DIFdel{$\delta(x)=\operatorname{dist}(x,\partial \Omega)$ }\DIFdelend \DIFaddbegin \DIFadd{we recall that $d(x)=\operatorname{dist}(x,\partial \Omega)$ }\DIFaddend and $\ell$ is given in \eqref{ell:def}.
...
\section{\DIFdelbegin \DIFdel{Localization of the $\mathbb{H}$-norm}\DIFdelend \DIFaddbegin \DIFadd{Preliminary results and properties}\DIFaddend }\DIFaddbegin \label{sec:three}

\subsection{\DIFadd{Localization of the $\mathbb{H}$-norm}}\DIFaddend \label{sec:localization-norm}
...
\end{align}
%
because if $\dist(x,\Omega)>1$ and $|x-y|<1$, then $y,x\not\in \Omega$ (\emph{i.e.}, $u(x)=u(y)=0$).  To \emph{localize} this norm \DIFdelbegin \DIFdel{, we }\DIFdelend \DIFaddbegin \DIFadd{we follow the ideas in \mbox{%DIFAUXCMD
\cite{Bor17}}\hspace{0pt}%DIFAUXCMD
, which are based on the works \mbox{%DIFAUXCMD
\cite{Fae00,Fae02}}\hspace{0pt}%DIFAUXCMD
. We }\DIFaddend introduce the following notation,
...
\begin{align*}
S_i:=\emptyset \qquad \text{ if }i\not\in\inter{0,N+1}\DIFaddbegin \DIFadd{.
}\DIFaddend \end{align*}
In particular, 
\begin{align*}
    S_i=\bigcup_
...
Let $L>0$ be fixed and $N\in\mathbb N$ be large enough \DIFdelbegin \DIFdel{such }\DIFdelend \DIFaddbegin \DIFadd{so }\DIFaddend that $h\in(0,\frac{1}{2})$. Then, there exists $C>0$ only depending on $L$ such that
%
   \begin{align*}\notag
    ...
    &\quad +C\sum_{i\in\{0,N\}}\int_{\Omega_i}|u(x)|^2\ell^{-1}(\DIFdelbegin \DIFdel{\delta}\DIFdelend \DIFaddbegin \DIFadd{d}\DIFaddend (x))\,dx
\end{align*}
%
...
%
\begin{equation*}
    I_1=\sum_{i\in\inter{0,N}}\left(\int_{\Omega_i}\int_{S_i}\frac{|u(x)-u(y)|^2}{|x-y|}\,dy\,dx+\int_{\Omega_i}\int_{D_i}\frac{|u(x)-u(y)|^2}{|x-y|}\,dy\,dx\right)\DIFaddbegin \DIFadd{,
}\DIFaddend \end{equation*}
%
where $D_i=\Omega\setminus S_i$. We keep the first integrals as they are. For the second ones, we have\DIFaddbegin \DIFadd{, }\DIFaddend by triangle inequality\DIFaddbegin \DIFadd{,
}\DIFaddend %
\begin{align*}\notag
...
\begin{align}\notag
    ...
    &\leq \DIFaddbegin \DIFadd{2}\DIFaddend \int_{\Omega_i}|u(x)|^2\int_{\frac{h}{2}}^{L}\rho^{-1}d\rho\,dx
    =\DIFaddbegin \DIFadd{2}\DIFaddend \left(-\log\left(\tfrac{h}{2L}\right)\right)\int_{\Omega_i}|u(x)|^2\,dx
...
%
\begin{equation*}
    I_2=\sum_{i\in\inter{0,N}}\int_{\Omega_i}|u(x)|^2\int_{B_2(\Omega)\setminus B_{h}(\Omega)}|x-y|^{-1}dy\,dx\DIFaddbegin \DIFadd{.
}\DIFaddend \end{equation*}
%
...
\begin{align}\label{eq:est_I3_init}
    I_3=H_0+H_N+\sum_{i\in\inter{1,N-1}}H_i\DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{,
}\DIFaddend \end{align}
%
...
%
\begin{align*}
    H_i&\leq \int_{\Omega_i}|u(x)|^2\int\DIFdelbegin \DIFdel{_{B_R(0)\setminus B_{\delta(x)}(x)}}\DIFdelend \DIFaddbegin \DIFadd{_{B_R(0)\setminus B_{d(x)}(x)}}\DIFaddend |x-y|^{-1}dy\,dx \\
    &\leq \int_{\Omega_i}|u(x)|^2\left(-\log\left(\DIFdelbegin %DIFDELCMD < \tfrac{\delta(x)}{R}%%%
\DIFdelend \DIFaddbegin \tfrac{d(x)}{R}\DIFaddend \right)\right)\,dx \leq C \int_{\Omega_i}|u(x)|^2\ell^{-1}(\DIFdelbegin \DIFdel{\delta}\DIFdelend \DIFaddbegin \DIFadd{d}\DIFaddend (x))\,dx\DIFaddbegin \DIFadd{,
}\DIFaddend \end{align*}
%
where $C>0$ only depends on $\Omega$. Therefore,
%
\begin{align}\label{eq:est_I3}
    I_3\leq C \sum_{i\in\{0,N\}}\int_{\Omega_i}|u(x)|^2\ell^{-1}(\DIFdelbegin \DIFdel{\delta}\DIFdelend \DIFaddbegin \DIFadd{d}\DIFaddend (x))\,dx + C\ell^{-1}(h)\sum_{\in\inter{1,N-1}}\int_{\Omega_i}|u(x)|^2\,dx\DIFaddbegin \DIFadd{.
}\DIFaddend \end{align}
%
...
\DIFaddbegin \subsection{\DIFadd{A density argument}}


\DIFaddend Recall the shape functions given in \eqref{eq:def_basis} and the space $\mathcal V_h$ defined in \eqref{Vh:def}. We define the (quasi-) interpolator $I_h:L^2(\Omega)\to \mathcal V_h$ by
%
...
%
\begin{equation*}
    |a_k|\leq h^{-1}|v|_{L^1(S_k)}\leq \sqrt{3}h^{-1/2}|v|_{L^2(S_k)}\DIFaddbegin \DIFadd{,
}\DIFaddend \end{equation*}
%
...
%
\begin{equation*}
    |v-\overline{v}|_{L^2(U)}^2\leq |v'|\DIFaddbegin \DIFadd{^2}\DIFaddend _{L^\infty(U)}|U|^3\DIFaddbegin \DIFadd{.
}\DIFaddend \end{equation*}
%
\end{lemma}
%
\begin{proof}
By definition and using Jensen's inequality\DIFdelbegin \DIFdel{we have
}\DIFdelend \DIFaddbegin \DIFadd{,
}\DIFaddend %
\begin{align*}
    \int_{U}|v(x)-\overline{v}|^2dx=\frac{1}{|U|^2}\int_{U}\left|\int_{U}(v(x)-v(y))dy\right|^2dx \leq \frac{1}{|U|}\int_{U}\int_{U}|v(x)-v(y)|^2dy dx\DIFaddbegin \DIFadd{,
}\DIFaddend \end{align*}
%
whence
%
...
\end{equation*}
%
By mean value theorem\DIFaddbegin \DIFadd{,
}\DIFaddend %
\begin{equation*}
    \int_{U}|v(x)-\overline{v}|^2dx 
    \leq |v'|\DIFaddbegin \DIFadd{^2}\DIFaddend _{L^\infty(U)}\int_{U}\int_{U}|x-y|dy dx
    \leq |v'|\DIFaddbegin \DIFadd{^2}\DIFaddend _{L^\infty(U)}|U|^3,
\end{equation*}
%
...
\begin{lemma}\label{lem:inter_l_infty}
    Let \DIFdelbegin \DIFdel{$v\in C_c^\infty(\Omega)$}\DIFdelend \DIFaddbegin \DIFadd{$v\in L^\infty(\Omega)$}\DIFaddend . Then, $|I_h v|_{L^\infty(\Omega)}\leq C|v|_{L^\infty(\Omega)}$ for some $C>0$ uniform with respect to $h$.
\end{lemma}
%
\begin{proof}
...
\leq \DIFaddbegin \DIFadd{C}\DIFaddend \sum_{k\in I_i^\prime}|a_k|^2|\Omega_i|
=\DIFdelbegin \DIFdel{h}\DIFdelend \DIFaddbegin \DIFadd{Ch}\DIFaddend \sum_{k\in I_i^\prime}|a_k|^2.
\end{align*}
%
\end{proof}
...
\begin{align}\label{eq:approxim_estimate}
    \int_{\Omega_i}\int_{S_i}\frac{|(v-I_h v)(x)-(v-I_h v)(y)|^2}{|x-y|}dy dx \leq C \DIFaddbegin \B{|v^\prime|^2_{\infty}} \DIFaddend h^3. 
\end{align}
\end{lemma}
%
\begin{proof}
...
\begin{equation}\label{eq:est_J1}
    J_1 = \int_{\Omega_i}\int_{S_i}\frac{|v(x)-v(y)|^2}{|x-y|}dy dx \leq C \int_{S_i}\int_{S_i}|x-y|dy dx \leq \DIFdelbegin \DIFdel{Ch}\DIFdelend \DIFaddbegin \DIFadd{C}\B{|v^\prime|^2_{\infty}}\DIFadd{h}\DIFaddend ^3,
\end{equation}
...
\begin{equation}\label{eq:est_bks}
    |b_k|^2\leq \DIFdelbegin \DIFdel{Ch}\DIFdelend \DIFaddbegin \DIFadd{C}\B{|v^\prime|^2_{\infty}}\DIFadd{h}\DIFaddend ^2.
\end{equation}
%
...
%
\begin{equation*}
    |v-I_hv|_{L^2(\Omega_i)} \leq C\DIFaddbegin \B{|v^\prime|^2_{\infty}} \DIFaddend h^3, \quad  v\in C_c^\infty(\Omega).
\end{equation*}
...
\leq \DIFdelbegin \DIFdel{Ch}\DIFdelend \DIFaddbegin \DIFadd{C}\B{|v^\prime|^2_{\infty}}\DIFadd{h}\DIFaddend ^3+Ch\sum_{k\in I_i^\prime}a_k^2,\label{a2}
\end{align}
...
%
\begin{equation*}
    |v-\overline{v}|_{L^2(U)}^2\leq \DIFdelbegin \DIFdel{Ch}\DIFdelend \DIFaddbegin \DIFadd{C}\B{|v^\prime|^2_{\infty}}\DIFadd{h}\DIFaddend ^3.
\end{equation*}
%
...
\begin{equation}\label{eq:rate_H_c_infty}
    \|v-I_h v\|_{\mathbb H(\Omega)}\leq C\DIFaddbegin \DIFadd{(|v|_{\infty},|v^\prime|_{\infty},L)}\DIFaddend \ell^\alpha(h).
\end{equation}
%
\DIFaddbegin \DIFadd{where $C(|v|_{\infty},|v^\prime|_{\infty},L)>0$ is a constant that only depends on $L$, $|v|_{\infty}$, $|v^\prime|_{\infty}$.
}\DIFaddend \end{proposition}

\begin{proof}
\DIFaddbegin \DIFadd{Along the proof, $C>0$ denotes possibly different constants that depend at most on $L$, $|v|_{\infty}$, $|v^\prime|_{\infty}$.
}

\DIFaddend Combining \eqref{eq:approxim_estimate} and \Cref{rem:l2_stability} with \Cref{lem:localization_enorm} we obtain
%
\begin{align*}
    \|v-I_h v\|\DIFaddbegin \DIFadd{^2}\DIFaddend _{\mathbb H(\Omega)}&\leq  C h^3(N+1)+Ch^3(N+1)\ell^{-1}(h)+C\sum_{i\in\{0,N\}}\int_{\Omega_i}|v-I_h v|^2\ell^{-1}(\DIFdelbegin \DIFdel{\delta}\DIFdelend \DIFaddbegin \DIFadd{d}\DIFaddend (x))\,dx \\
    &\leq Ch^2+Ch^2\ell^{-1}(h)+C\sum_{i\in\{0,N\}}\int_{\Omega_i}|v-I_h v|^2\ell^{-1}(\DIFdelbegin \DIFdel{\delta}\DIFdelend \DIFaddbegin \DIFadd{d}\DIFaddend (x))\,dx\DIFaddbegin \DIFadd{,
}\DIFaddend \end{align*}
\DIFdelbegin \DIFdel{since }\DIFdelend \DIFaddbegin \DIFadd{because }\DIFaddend $h=\frac{L}{N+1}$. Since $v\in C_c^\infty(\Omega)$, a direct computation and \Cref{lem:inter_l_infty} yield
%
\begin{align*}
    \|v-I_h v\|\DIFaddbegin \DIFadd{^2}\DIFaddend _{\mathbb H(\Omega)} &\leq Ch^2+Ch^2\ell^{-1}(h)+C\left(\|v\|_{L^\infty(\Omega)}^2+\|I_hv\|^2_{L^\infty(\Omega)}\right)\sum_{i\in\{0,N\}}\int_{\Omega_i}\ell^{-1}(\DIFdelbegin \DIFdel{\delta}\DIFdelend \DIFaddbegin \DIFadd{d}\DIFaddend (x))\,dx \\
    & \leq Ch^2+Ch^2\ell^{-1}(h)+C\ell^{\alpha}(h)\sum_{i\in\{0,N\}}\int_{\Omega_i}\ell^{-1-\alpha}(\DIFdelbegin \DIFdel{\delta}\DIFdelend \DIFaddbegin \DIFadd{d}\DIFaddend (x))\,dx.
\end{align*}
%
Using Lemma~\ref{lem:ellbeta} and \DIFdelbegin \DIFdel{since }\DIFdelend \DIFaddbegin \DIFadd{that }\DIFaddend $h\leq \ell^{\frac{1+\alpha}{2}}(h)$ for any $h\leq h_0$ for some $h_0>0$ only depending on $\alpha$, we obtain
%
\begin{align*}
    \|v-I_h v\|\DIFaddbegin \DIFadd{^2}\DIFaddend _{\mathbb H(\Omega)}
    & \leq C\ell^{1+\alpha}(h)+C\ell^{\alpha}(h)\leq C \ell^{\alpha}(h),
\end{align*}
which is the desired result \DIFaddbegin \B{(after adjusting the value of $\alpha$)}\DIFaddend .
\end{proof}
...
\begin{align*}
    ...
    &\leq \frac{2\epsilon}{3}+\|w-I_hw\|_{\mathbb H(\Omega)} \DIFdelbegin \DIFdel{. 
}\DIFdelend \DIFaddbegin \DIFadd{\leq \frac{2\epsilon}{3}+}\B{C(|w|_{\infty},|w^\prime|_{\infty},L)}\DIFadd{\ell^\alpha(h),
}\DIFaddend \end{align*}
%
where we have used that $\|\Pi_h\|_*=1$\DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend property \eqref{eq:prop_infimo}\DIFdelbegin \DIFdel{. Lastly, using \mbox{%DIFAUXCMD
\Cref{eq:estab_H_c_infty} }\hspace{0pt}%DIFAUXCMD
with $h>0$ small enough, $\|w-I_hw\|_{\mathbb H(\Omega)}\leq \epsilon/3$ and therefore $\|v-v_h\|_{\mathbb H(\Omega)}\leq \epsilon$}\DIFdelend \DIFaddbegin \DIFadd{, and \mbox{%DIFAUXCMD
\Cref{eq:estab_H_c_infty}}\hspace{0pt}%DIFAUXCMD
. }\B{Finally, we choose $h>0$ small enough so that ${C(|w|_{\infty},|w^\prime|_{\infty},L)}\ell^\alpha(h)\leq \frac{\epsilon}{3}$ (note that $w\in C^\infty_c(\Omega)$ is independent of $h$)}\DIFaddend . This yields the claim.
\end{proof}
...
\begin{equation}\label{eq:est_J2_weight}
    J_2 \leq Ch\sum_{k\in I_i} b_k^2\DIFaddbegin \DIFadd{,
}\DIFaddend \end{equation}
%
...
\end{align*}
because the support of $\varphi_k$ is in $T_{i}$ for all $k\in I_i$. Using \Cref{lem:poincare_type_weight}, we can estimate the right-hand side of the above expression and obtain
%
...
\end{proposition}
\begin{proof}
By \Cref{lem:localization_enorm} and estimates \eqref{eq:approxim_estimate_weight}-\eqref{a4} in \Cref{lem:stab_dif_interp_weight}, we have
%
\begin{align}
...
\end{align}
where we used that $\frac{\ell^{\alpha}(d(x))}{\ell^{\alpha}(h)}\leq 1$ for $x\in\Omega_i$ \DIFaddbegin \DIFadd{with }\B{$i\in\{0,N\}$}\DIFaddend .  
...
\begin{equation}\label{eq:discr_problem}
    \cE_{L}(u_h,v)=\int_{\Omega} f v \quad \text{ for all } v\in \mathcal V_h\DIFaddbegin \DIFadd{.
}\DIFaddend \end{equation}
%


Unlike the case of the fractional Laplacian, we observe that the bilinear form $\cE_{L}(u,u)$ is not coercive in $\mathbb H(\Omega)$ so we cannot employ the classical Cea's lemma. Instead, we shall use the following variant which employs a discrete inf-sup condition. \DIFaddbegin \DIFadd{Some of the arguments shown below are well-known and standard in other settings, but we include proofs for completeness.
}\DIFaddend %
...
\begin{equation}\label{eq:discr_inf_sup}
    \sup_{v\in \mathcal V_h}\frac{\cE_{L}(u_h,v)}{\|v\|_{\mathbb H(\Omega)}}\geq \alpha_0\|u_h\|_{\mathbb H(\Omega)} \DIFdelbegin \DIFdel{, }\DIFdelend \quad\text{ for all } u_h\in\mathcal V_h\DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{,
}\DIFaddend \end{equation}
%
\DIFdelbegin \DIFdel{Then, there exists }\DIFdelend \DIFaddbegin \DIFadd{then there is }\DIFaddend a unique solution $u_h\in \mathcal V_h$ to \eqref{eq:discr_problem}. Moreover, this solution satisfies
%
\begin{equation}\label{eq:stability_cea}
    \|u-u_h\|_{\mathbb H(\Omega)}\leq \left(1+\frac{C}{\alpha_0}\right)\inf_{w\in\mathcal V_h}\|u-w\|_{\mathbb H(\Omega)}\DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{,
}\DIFaddend \end{equation}
%
...
%
\begin{equation*}
    |\cE_{L}(u-w,v)|\leq C\|u-w\|_{\mathbb H(\Omega)}\|v\|_{\mathbb H(\Omega)}\DIFaddbegin \DIFadd{,
}\DIFaddend \end{equation*}
%
whence
%
...
Fix $\alpha\in(0,1)$ as given by \Cref{eq:regularity}. Using \Cref{prop:est_interpolator} with this particular $\alpha$ and estimate \eqref{eq:stability_cea} we get
%
\begin{align*}
...
In \cite{L22}, a \DIFdelbegin \DIFdel{good }\DIFdelend numerical approximation of these coefficients is done using a trapeze formula.  The error involved in this approximation is of the order $O(\frac{1}{N^2})$.  
...
\DIFdelend 2\int_{h}^{+\infty}\int_{0}^{h} \ldots\,dxdy  + \int_{0}^{h}\int_{0}^{h} \ldots\,dxdy + 2\int_{-\infty}^{0}\int_{0}^{h} \ldots\,dxdy,
\end{align*}
where 

% \subsubsection*{Computation of $R_2$}
\begin{align*}
	R_1 &:= 2\int_{h}^{+\infty}\int_{0}^{h} \frac{\varphi_0^2(x)}{|x-y|^{1+2s}}\,dxdy = 4\int_{0}^{h}\left(1-\frac{x}{h}\right)^2\left(\int_{h}^{+\infty} \frac{1}{(y-x)^{1+2s}}\, dy\right)\,dx\DIFdelbegin %DIFDELCMD < \\
%DIFDELCMD < 	&%%%
\DIFdelend = \frac{2}{s}\int_{0}^{h}\frac{\left(1-\frac{x}{h}\right)^2}{(h-x)^{2s}}\,dx
...
%
\begin{equation*}
    a_{0,0}=\frac{4h^{1 - 2 s}}{s (3 - 2 s) (1 - 2 s)}\DIFaddbegin \DIFadd{.
}\DIFaddend \end{equation*}
%
...
Therefore,
\begin{align*}
	a_{0,j} =-\frac{h^{1-2 s} \gamma_j^s }{\sqrt{2}(1-2 s) (1-s) s (3- 2s )}\DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{,
}\DIFaddend \end{align*} 
where
%
...
%
\begin{align*}
a_{0,N+1} = -\frac{h^{1-2 s} \zeta_N^s }{s(1-s)(1-2s )(3-2 s)}\DIFaddbegin \DIFadd{,
}\DIFaddend \end{align*}
%
where 
%
...
\DIFdelbegin \DIFdel{Le }\DIFdelend \DIFaddbegin \DIFadd{Let }\DIFaddend us now compute the terms $Q_i$, $i=1,\ldots,6$, separately.   Note that $Q_6=Q_3=Q_1 = 0.$ Moreover, by Fubini's theorem,
\begin{align*}
...
%DIFDELCMD < \displaystyle -\frac{2 \gamma }{3}+\frac{16}{9}-\frac{4}{3}  \log (h)+\frac{4}{3}  \log
%DIFDELCMD <    (2)+\frac{2}{3}  \psi \left(\frac{1}{2}\right), & \textnormal{$i=0$, $j=0$,} \\
%DIFDELCMD < \displaystyle -\frac{\gamma }{3\sqrt{2}}+\frac{5}{9\sqrt{2}}-\frac{\sqrt{2}}{3} \log (h)-\frac{\sqrt{2}}{3}  \log
%DIFDELCMD <    (2)+\frac{1}{3\sqrt{2}} \psi \left(\frac{1}{2}\right), & \textnormal{$i=0$, $j=1$,} \\
%DIFDELCMD <    -\frac{\sqrt {2}}{3},
%DIFDELCMD <  & \parbox[t]{.3\columnwidth}{$i=0$, $j=2$}\\
%DIFDELCMD <  \mathsf{l}_k, 
%DIFDELCMD <  & \parbox[t]{.3\columnwidth}{$i=0$, $j\in\inter{3,N}$}\\
%DIFDELCMD <  \mathsf{m}_k, &  \parbox[t]{.3\columnwidth}{$i=0$, $j=N+1$,}\\
%DIFDELCMD < \displaystyle -\frac{\mathsf p_k}{3}, & \textnormal{$i,j\in\inter{1,N}$ and $j-i=2$,} \\
%DIFDELCMD < \displaystyle \frac{\mathsf q_k}{6}, & \textnormal{$i,j\in\inter{1,N}$ and $j-i> 2$,} \\
%DIFDELCMD < \displaystyle -\frac{\gamma}{6} + \frac{11}{18} - \frac{\log(h)}{3}  + \frac{64\log(2) - 54\log(3)}{12} + \frac{\log(2)}{3}  + \frac{1}{6} \psi(\frac 12), & \textnormal{$i\in\inter{1,N-1}$ and $j=i+1$,} \\
%DIFDELCMD < \displaystyle -\frac{2}{9}\left[6\log(h) + 3\gamma - 11 + \log(64) - 3\psi(\frac{1}{2})\right], & \textnormal{$i\in\inter{1,N}$ and $j=i$.}
%DIFDELCMD < \end{cases}%%%
...
\displaystyle -\frac{2}{9}\left[6\log(h) + 3\gamma - 11 + \log(64) - 3\psi(\frac{1}{2})\right], & \textnormal{$i\in\inter{1,N}$ and $j=i$,}
\end{cases}\DIFaddend 
\end{equation}
%
where 
%
\begin{align*} \notag
\mathsf{l}_k:= & \DIFdelbegin \DIFdel{\frac{1}{3\sqrt{2}}  }%DIFDELCMD < \left[%%%
\DIFdel{-3 j^3 \log (j)+6 j^2 \log (j)+(j-2)^3 (-\log (j-2))+3
   (j-1)^2 (j-2) \log (j-1) }%DIFDELCMD < \right. \\
%DIFDELCMD <    &\left. %%%
\DIFdel{+(j+1)^2 (j-2) \log (j+1)-2}%DIFDELCMD < \right] %%%
\DIFdelend \DIFaddbegin \DIFadd{\frac{-3 j^3 \log (j)+6 j^2 \log (j)+(j-2)^3 (-\log (j-2))+3
   (j-1)^2 (j-2) \log (j-1)+(j+1)^2 (j-2) \log (j+1)-2}{3\sqrt{2}}, }\DIFaddend \\
   \mathsf{m}_k:= & \frac{1}{3}\left[2 (N-3) N^2 \log (N)-N-(N-1)^3 \log (N-1)-(N+1) ((N-4) N+1) \log (N+1)-3\right]\DIFaddbegin \DIFadd{, }\DIFaddend \\
\mathsf{p}_k:=&  -24\log(3) - 12\log(4) - 16\log(9) + 27\log(16) + \log(144)\DIFaddbegin \DIFadd{, }\DIFaddend \\ \notag
\mathsf{q}_k:=&-(-2 + k)^3\log(-2 + k) + 4(-1 + k)^3\log(-1 + k) - 6 k^3 \log(k) \DIFdelbegin %DIFDELCMD < \\
%DIFDELCMD < &%%%
\DIFdelend +4(1 + k)^3\log(1 + k) - (2 + k)^3\log(2 + k)\DIFaddbegin \DIFadd{,
}\DIFaddend \end{align*}
\DIFdelbegin \DIFdel{and }\DIFdelend $\psi$ is the digamma function\DIFaddbegin \DIFadd{, }\DIFaddend and $\gamma=-\psi(1)$\DIFaddbegin \DIFadd{.
}\DIFaddend 
...
\subsection{\DIFdelbegin \DIFdel{On }\DIFdelend \DIFaddbegin \DIFadd{Some numerical experiments on }\DIFaddend the optimality of the convergence rate}

In general, a \DIFdelbegin \DIFdel{good }\DIFdelend \DIFaddbegin \DIFadd{usual }\DIFaddend way to observe the optimality of the FEM convergence rate is using the torsion function, namely the solution of the boundary-value problem with right-hand side $f\equiv 1$ (see, among others, \cite[Section 5]{AB17} or \cite[Section 5]{BHS19})\DIFaddbegin \DIFadd{, which in many cases has an explicit formula}\DIFaddend . Unfortunately, no closed expression for the torsion function of the logarithmic Laplacian \eqref{t:p:intro} is known.
...
\end{align}
where $\chi_{\Omega}$ denotes the characteristic function in the interval $(-1,1)$.  \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend This choice is justified by the following facts
\begin{itemize}
 \item $u$ has a uniformly bounded logarithmic Laplacian in $(-1,1)$ (as a direct nontrivial calculation shows\DIFdelbegin \DIFdel{),
 }\DIFdelend \DIFaddbegin \DIFadd{, see \mbox{%DIFAUXCMD
\cite{HSLRS23}}\hspace{0pt}%DIFAUXCMD
). Nonetheless, we remark that $L_{\Delta} u$ does not have a closed formula;
 }\DIFaddend \item \DIFaddbegin \DIFadd{$u$ }\DIFaddend has the optimal boundary regularity\footnote{This is needed to observe the optimal convergence rate. Indeed, solutions which are smoother typically yield better convergence rates, see, for instance, \cite[Section 5 and Table 2]{AB17} for the case of the fractional Laplacian.} for logarithmic Dirichlet problems \DIFaddbegin \DIFadd{(see \mbox{%DIFAUXCMD
\cite{HSLRS23}}\hspace{0pt}%DIFAUXCMD
)}\DIFaddend , namely, it behaves as $\ell^{1/2}(d(x))$\DIFdelbegin \DIFdel{(see the Introduction for the definition of $d$ and $\ell$)}\DIFdelend .
\end{itemize}
Here we only observe these facts numerically (see Figures~\ref{fig:1} and~\ref{fig:2}), but we we refer to \cite{HSLRS23} for a detailed proof. \DIFdelbegin \DIFdel{See also \mbox{%DIFAUXCMD
\cite[Theorem 1.11]{CW19} }\hspace{0pt}%DIFAUXCMD
and \mbox{%DIFAUXCMD
\cite[Remark~1.5]{FJW22}}\hspace{0pt}%DIFAUXCMD
.
}\DIFdelend %DIF > See also \cite[Theorem 1.11]{CW19} and \cite[Remark~1.5]{FJW22}.
...
\begin{figure}[htbp]
	...
	\caption{Logarithmic Laplacian \DIFaddbeginFL \DIFaddFL{of $u(x)$ as in }\eqref{udef}\DIFaddendFL .}
	\label{fig:1}
\end{figure}

Now, consider $h=\frac{1}{N+1}$ for $N\in\mathbb N$, and let $x_i:=-1+ih$ for $i\in\inter{0,N+1}$ so that $-1=x_0<x_1<\ldots<x_{N}<x_{N+1}=1$. Let \DIFdelbegin \DIFdel{$f_i:=h L_\Delta u(x_i)$ }\DIFdelend \DIFaddbegin \DIFadd{$f_i:=h [L_\Delta u](x_i)$ }\DIFaddend for $i\in\inter{0,N+1}$, $F:=(f_i)$, and let $\alpha:=A_h^{-1}F$, 
...
\end{align}
which is a \DIFdelbegin \DIFdel{good approximation for }\DIFdelend \DIFaddbegin \DIFadd{way to approximate }\DIFaddend $u_h$ (defined in Theorem~\ref{main:thm:intro}), \DIFaddbegin \DIFadd{namely, $f_i\approx \int_{\Omega}L_{\Delta}u(x)\phi_i(x)dx$, }\DIFaddend see Figure~\ref{fig:2}. \DIFaddbegin \DIFadd{Since there is no closed formula for $L_{\Delta}u$ we compute $f_i$ with the aid of Mathematica 14.0, where }\eqref{LL} \DIFadd{is computed and approximated numerically on the mesh grid points $x_i$.
}\DIFaddend 
...
\begin{figure}[htb]
	\centering
	%
	\DIFdelbeginFL %DIFDELCMD < \begin{tikzpicture}[scale=1]
%DIFDELCMD < 	\begin{axis}[xlabel={$x$},xmin=-1.001,xmax=1.001,legend pos=outer north east,
%DIFDELCMD < 	legend plot pos=left, xtick={-1,0,1},
%DIFDELCMD < 	legend style={cells={anchor=west},draw=none}]
%DIFDELCMD < 	%	    
%DIFDELCMD < 	\pgfplotstableread{sol-log_num-vs-exact_20-06-2023_12h48.org}\solution   
%DIFDELCMD < 	\pgfplotstableread{sol-log_num-vs-exact_21-06-2023_16h18.org}\solutionfine         
%DIFDELCMD < 	      

%DIFDELCMD < 	\addplot[thick,color=blue] table[x=xi,y=exsol] \solutionfine; %\label{cost}
%DIFDELCMD < 	 \addlegendentry{$u$};
%DIFDELCMD < 	\addplot[thick,color=red,every mark/.append style={solid, fill=none}, mark=x] table[x=xi,y=numsol] \solution; %\label{cost}
%DIFDELCMD < 	 \addlegendentry{$v_h$};
%DIFDELCMD < 	
...
\DIFaddendFL 
	...
	\caption{A comparison between $u$ given by \eqref{udef} and \DIFdelbeginFL \DIFdelFL{$v$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$v_h$ }\DIFaddendFL given by \eqref{vdef} for $N=50$.}
	\label{fig:2}
\end{figure}
...
\begin{figure}[htb]
	...
	legend style={cells={anchor=west},draw=none}]
	%	    
	\pgfplotstableread{sol-log_convergence_data_20-06-2023_13h27.org}\solution      
	...
	legend style={cells={anchor=west},draw=none}]
	%	    
	\pgfplotstableread{sol-log_convergence_data_20-06-2023_13h27.org}\data      
	...
	legend style={cells={anchor=west},draw=none}]
	%	    
	\pgfplotstableread{sol-log_convergence_data_21-06-2023_11h34.org}\data      
...
Note that these curves are not straight lines. This contrasts with the typical behavior of algorithms with convergence rate of order $h^p$ for some $p>0$; in particular, this is the case of the fractional Laplacian, see for instance \cite[Figure 8]{BH17}, \cite[Figure 1]{AB17}, and Table~\ref{table:frac_lap} in the Appendix. \DIFdelbegin \DIFdel{This suggests that the optimal convergence rate for the logarithmic Laplacian is }\emph{\DIFdel{not}} %DIFAUXCMD
\DIFdel{of power-type, as suggested in Theorem~\ref{main:thm:intro}.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend Although the curve in Figure~\ref{l2_ah} looks almost straight, a computation of its slope (with respect to consecutive points) shows otherwise, as presented in Table~\ref{table:1}.

\DIFdelbegin %DIFDELCMD < \begin{table}[htb]
%DIFDELCMD < \centering
%DIFDELCMD < %%%
%DIF < 
%DIFDELCMD < \pgfplotstableread{sol-log_convergence_data_21-06-2023_11h34.org}\data
%DIFDELCMD < %%%
%DIF < 
%DIFDELCMD < \pgfplotstableset{
%DIFDELCMD < columns={N,h,L2norm,slopeL2,L2locnorm,slopeL2loc,Linfnorm,slopeinf},
%DIFDELCMD < %%%
%DIFDELCMD < columns/N/.style={
%DIFDELCMD < column name=$N$,
%DIFDELCMD < },	
%DIFDELCMD < %%% 
%DIFDELCMD < columns/h/.style={
%DIFDELCMD < column name=$h$,
%DIFDELCMD < sci,sci zerofill,sci subscript,
%DIFDELCMD < precision=2,
%DIFDELCMD < column type/.add={}{|}},	
%DIFDELCMD < %%%
%DIFDELCMD < columns/L2norm/.style={
%DIFDELCMD < column name=$L^2$--norm ($a_h$),
%DIFDELCMD < sci,sci zerofill,sci subscript,
%DIFDELCMD < precision=3},
%DIFDELCMD < %%%
%DIFDELCMD < columns/slopeL2/.style={
%DIFDELCMD < column name=slope,
%DIFDELCMD < sci,sci zerofill,sci subscript,
%DIFDELCMD < precision=2,
%DIFDELCMD < column type/.add={}{|}},	
%DIFDELCMD < %%%
%DIFDELCMD < columns/L2locnorm/.style={
%DIFDELCMD < column name=$L^2_{loc}$-norm ($b_h$),
%DIFDELCMD < sci,sci zerofill,sci subscript,
%DIFDELCMD < precision=3},
%DIFDELCMD < %%%
%DIFDELCMD < columns/slopeL2loc/.style={
%DIFDELCMD < column name=slope,
%DIFDELCMD < %sci,sci zerofill,sci subscript,
%DIFDELCMD < precision=2,
%DIFDELCMD < column type/.add={}{|}},	
%DIFDELCMD < %%%
%DIFDELCMD < columns/Linfnorm/.style={
%DIFDELCMD < column name=$L_{\infty}$--norm ($c_h$),
%DIFDELCMD < sci,sci zerofill,sci subscript,
%DIFDELCMD < precision=4,
%DIFDELCMD < column type/.add={}{}},
%DIFDELCMD < %%%
%DIFDELCMD < columns/slopeinf/.style={
%DIFDELCMD < column name=slope,
%DIFDELCMD < sci,sci zerofill,sci subscript,
%DIFDELCMD < precision=2,
%DIFDELCMD < column type/.add={}{}},
...
%DIFDELCMD < %%%
%DIF < 
%DIFDELCMD < \pgfplotstabletypeset[clear infinite, empty cells with={\ensuremath{-}},
%DIFDELCMD < every head row/.style={
%DIFDELCMD < before row=\toprule,after row=\midrule},
%DIFDELCMD < every last row/.style={
%DIFDELCMD < after row=\bottomrule},
%DIFDELCMD < ]{\data}
%DIFDELCMD < %%%
%DIF < 
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Error data  for $a_h$, $b_h$, and $c_h$.}}
%DIFAUXCMD
...
%DIFDELCMD < %%%
\DIFdelend We mention that this behavior is robust with respect to the size of the interval.  Below we present the same experiment for the function 
$w(x)=\left(
...
\DIFaddbegin \DIFadd{The discussion above suggests that the optimal convergence rate for the logarithmic Laplacian is }\emph{\DIFadd{not}} \DIFadd{of power-type. This is consistent with the rate shown in Theorem~\ref{main:thm:intro}. Intuitively, the optimal error rate has an intrinsic connection with the optimal regularity of weak solutions, which in this case is of logarithmic type (see \mbox{%DIFAUXCMD
\cite[Theorem 1.2]{HSLRS23}}\hspace{0pt}%DIFAUXCMD
). Therefore, we do not know if the error rate can be enhanced (e.g. to a polynomial rate), even with the use of standard techniques employed to improve convergence rates (such as graded meshes}\footnote{\DIFadd{For more details on the use of graded meshes in nonlocal problems we refer to \mbox{%DIFAUXCMD
\cite[Section 3.4]{Bor17}}\hspace{0pt}%DIFAUXCMD
, where  of weighted Sobolev spaces are used to improve the error; however, we remark that there is no analogue of these results for the logarithmic Laplacian, for which the regularity theory is not well understood yet, see the discussion in \mbox{%DIFAUXCMD
\cite[Section 2]{CS22}}\hspace{0pt}%DIFAUXCMD
.  Graded meshes where implemented numerically in \mbox{%DIFAUXCMD
\cite{L22} }\hspace{0pt}%DIFAUXCMD
for the logarithmic Laplacian.}}\DIFadd{). More research in this direction and a better understanding of the regularity of logarithmic problems is required to address these interesting open problems.
}
...
%DIF > 
\caption{\DIFaddFL{Error data  for $a_h$, $b_h$, and $c_h$.}}
\label{table:1}
\end{table}
...
\centering
%
\pgfplotstableread{sol-log_convergence_data_21-06-2023_16h57.org}\data
%
\pgfplotstableset{
columns={N,h,L2norm,slopeL2,L2locnorm,slopeL2loc,Linfnorm,slopeinf},
%%%
...
Now\DIFaddbegin \DIFadd{, }\DIFaddend we use three explicit solutions to further illustrate the FEM approximation of solutions to logarithmic Dirichlet problems. For \DIFdelbegin \DIFdel{$r>0$}\DIFdelend \DIFaddbegin \DIFadd{$L>0$}\DIFaddend , let  $\Omega:=(-L,L)$, $h_\Omega(x):=-\ln(L^2-|x|^2)$, and $\rho_1:=2 \ln (2)+\psi\left(\frac{1}{2}\right)-\gamma\approx -1.15443$, where $\gamma=-\Gamma^{\prime}(1)$ is the Euler-Mascheroni constant and $\psi$ is the Digamma function given by $\psi=\frac{\Gamma^{\prime}}{\Gamma}$.  For $x\in\R$, let 
\begin{align*}
u_1(x):=\chi_{[-L,L]}(x), \qquad
u_2(x):=\chi_{[-L,L]}(x) x, \qquad
u_3(x):=(L^2-x^2)_+=(L^2-x^2)\chi_{[-L,L]}(x).
\end{align*}
Then, for $x\in \Omega$, \DIFdelbegin \begin{align*}
    \DIFdel{L_\Delta u_1(x)}&\DIFdel{=h_\Omega(x)+\rho_1,}\\
    \DIFdel{L_\Delta u_2(x)}&\DIFdel{=x(2+h_\Omega(x)+\rho_1),}\\
    \DIFdel{L_\Delta u_3(x)}&\DIFdel{=L^2-3x^2+(h_\Omega(x)+\rho_1)(L^2-x^2).
}\end{align*}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{By \mbox{%DIFAUXCMD
\cite[Corollary 1.9]{CW19} }\hspace{0pt}%DIFAUXCMD
it is stated that the (weak) maximum principle holds in 1D whenever 
}\begin{align*}
    \DIFdel{2L=|\Omega|\leq 2 e^{\frac{1}{2}(\psi(\frac{1}{2})-\gamma)}|B_1(0)|=4e^{\frac{1}{2}(\psi(\frac{1}{2})-\gamma)},
    }\end{align*}%DIFAUXCMD
%DIFDELCMD <     %%%
\DIFdel{that
is,
    }\DIFdelend \DIFaddbegin \DIFadd{it is not difficult to check that
}\DIFaddend \begin{align*}
    L\DIFdelbegin \DIFdel{\leq }\DIFdelend \DIFaddbegin \DIFadd{_\Delta u_1(x)}&\DIFadd{=h_\Omega(x)+\rho_1,}\\
    \DIFadd{L_\Delta u_2(x)}&\DIFadd{=x(}\DIFaddend 2\DIFdelbegin \DIFdel{e^{\frac{1}{2}(\psi(\frac{1}{2})-\gamma)}\approx 0.6453}\DIFdelend \DIFaddbegin \DIFadd{+h_\Omega(x)+\rho_1),}\\
    \DIFadd{L_\Delta u_3(x)}&\DIFadd{=L^2-3x^2+(h_\Omega(x)+\rho_1)(L^2-x^2)}\DIFaddend .
\end{align*}

%DIF > By \cite[Corollary 1.9]{CW19} it is stated that the (weak) maximum principle holds in 1D whenever 
%DIF > \begin{align*}
%DIF >     2L=|\Omega|\leq 2 e^{\frac{1}{2}(\psi(\frac{1}{2})-\gamma)}|B_1(0)|=4e^{\frac{1}{2}(\psi(\frac{1}{2})-\gamma)},
%DIF >     \end{align*}
%DIF >     that is,
%DIF >     \begin{align*}
%DIF >         L\leq 2 e^{\frac{1}{2}(\psi(\frac{1}{2})-\gamma)}\approx 0.6453.
%DIF > \end{align*}
\DIFaddbegin 
...
\begin{figure}[htb]
	\centering
	%
	\DIFdelbeginFL %DIFDELCMD < \subfloat[$u_1$ and its FEM approximation.]{
%DIFDELCMD < 	\begin{tikzpicture}[scale=0.8]
%DIFDELCMD < 	\begin{axis}[xlabel={$x$},xmin=-1,xmax=1,legend pos=outer north east,
%DIFDELCMD < 	legend plot pos=left, xtick={-1,0,1},
%DIFDELCMD < 	legend style={cells={anchor=west},draw=none}]
%DIFDELCMD < 	%	    
%DIFDELCMD < 	\pgfplotstableread{sol-log_num-vs-exact_20-06-2023_16h03.org}\solution      
%DIFDELCMD < 	      

%DIFDELCMD < 	\addplot[thick,color=blue] table[x=xi,y=exsol] \solution; %\label{cost}
%DIFDELCMD < 	\addplot[thick,color=red,every mark/.append style={solid, fill=none}, only marks, mark=x] table[x=xi,y=numsol] \solution; %\label{cost}
%DIFDELCMD < 	
...
%DIFDELCMD < 	%%%
\DIFdelendFL %
	\DIFdelbeginFL %DIFDELCMD < \subfloat[$u_2$ and its FEM approximation.]{
%DIFDELCMD < 	\begin{tikzpicture}[scale=0.8]
%DIFDELCMD < 	\begin{axis}[xlabel={$x$},xmin=-1,xmax=1,ymin=0.0,legend pos=outer north east,
%DIFDELCMD < 	legend plot pos=left, xtick={-1,0,1},
%DIFDELCMD < 	legend style={cells={anchor=west},draw=none}]
%DIFDELCMD < 	%	    
%DIFDELCMD < 	\pgfplotstableread{sol-log_num-vs-exact_20-06-2023_16h01.org}\solution      
%DIFDELCMD < 	      
...
%DIFDELCMD < 	\end{tikzpicture}
%DIFDELCMD < 	}\\ %%%
\DIFdelendFL \DIFaddbeginFL \subfloat[$u_1$ and its FEM approximation.]{
	\begin{tikzpicture}[scale=0.8]
	...
	\DIFdelbeginFL %DIFDELCMD < \subfloat[The function $u_3$ and its FEM approximation.]{
%DIFDELCMD < 	\begin{tikzpicture}[scale=0.8]
%DIFDELCMD < 	\begin{axis}[xlabel={$x$},xmin=-1,xmax=1,legend pos=outer north east,
%DIFDELCMD < 	legend plot pos=left, xtick={-1,0,1},
%DIFDELCMD < 	legend style={cells={anchor=west},draw=none}]
%DIFDELCMD < 	%	    
%DIFDELCMD < 	\pgfplotstableread{sol-log_num-vs-exact_20-06-2023_15h59.org}\solution      
%DIFDELCMD < 	      

%DIFDELCMD < 	\addplot[thick,color=blue] table[x=xi,y=exsol] \solution; %\label{cost}
%DIFDELCMD < 	\addlegendentry{Exact solution};
%DIFDELCMD < 	\addplot[thick,color=red,every mark/.append style={solid, fill=none}, only marks, mark=x] table[x=xi,y=numsol] \solution; %\label{cost}
%DIFDELCMD < 	 \addlegendentry{Numerical approximation};
%DIFDELCMD < 	
...
%DIFDELCMD < 	} 
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \subfloat[The function $u_2$ and its FEM approximation.]{
	\begin{tikzpicture}[scale=0.8]
	...
	\DIFaddendFL \caption{Exact solutions and their numerical approximations for $N=50$.}
	\label{fig:comp_sol}
\end{figure}

We remark that the functions $u_1$ and $u_2$ do not \DIFdelbegin \DIFdel{satify }\DIFdelend \DIFaddbegin \DIFadd{satisfy }\DIFaddend the assumptions of Theorem~\ref{main:thm:intro}, because the right-hand side does not belong to $\mathcal Y(\Omega)$. Nevertheless, the FEM \DIFdelbegin \DIFdel{yields a good }\DIFdelend \DIFaddbegin \DIFadd{produces an }\DIFaddend approximation also in these cases\DIFdelbegin \DIFdel{.  On the other hand, }\DIFdelend \DIFaddbegin \DIFadd{, see \mbox{%DIFAUXCMD
\Cref{fig:comp_sol} }\hspace{0pt}%DIFAUXCMD
and \mbox{%DIFAUXCMD
\Cref{table:u1u2u3} }\hspace{0pt}%DIFAUXCMD
for further information on the quality of approximation.  We shall mention that }\DIFaddend the function $u_3$ is Lipschitz in $\R$ and thus it exhibits a better convergence rate. Therefore, we cannot use $u_3$ to show the optimality of the convergence rate stated in Theorem~\ref{main:thm:intro}.

\DIFaddbegin \begin{table}[htb!]
\centering
%DIF > 
\pgfplotstableread{sol-log_convergence_data_07-08-2024_15h10.org}\data
%DIF > 
\pgfplotstableset{
...
%DIF > 
\pgfplotstableread{sol-log_convergence_data_07-08-2024_15h45.org}\data
%DIF > 
\pgfplotstableset{
...
%DIF > 
\pgfplotstableread{sol-log_convergence_data_07-08-2024_15h03.org}\data
%DIF > 
\pgfplotstableset{
columns={L2norm,slopeL2},
%%%
...
%DIF > 
\caption{\DIFaddFL{Error data for the approximation of $u_1$, $u_2$ and $u_3$.}}
\label{table:u1u2u3}
\end{table}


\DIFaddend \section{Approximation of the eigenvalues of the logarithmic Laplacian}\label{eigenvalue approximation:sec}
...
% 
\begin{align}\notag 
\mathcal{E}_L(m_h-m,m_h+m)&\leq \mathcal{E}(m_h-m,m_h+m)\DIFdelbegin %DIFDELCMD < \\ \notag
%DIFDELCMD < &\quad %%%
\DIFdelend +\|m_h-m\|_{L^1(\R)}\|m_h+m\|_{L^1(\R)}+\rho_1\|m_h-m\|_{L^2(\R)}\|m_h+m\|_{L^2(\R)}\notag\\
&\leq \|m_h-m\|_{\mH(\Omega)}\Big(\|m_h\|_{\mH(\Omega)}+\|m\|_{\mH(\Omega)}\Big)\DIFdelbegin %DIFDELCMD < \\
%DIFDELCMD < &\quad %%%
\DIFdelend +\Big(|\Omega|^2+\rho_1\Big)\|m_h-m\|_{L^2(\R)}\Big(\|m_h\|_{L^2(\R)}+\|m\|_{L^2(\R)}\Big)\notag\\
...
%
\begin{equation*}
    \int_{0}^{R}\frac{\ell^{1+\alpha}(\rho)}{\rho}d\rho=\int_{0}^{r} \frac{\ell^{1+\alpha}(\rho)}{\rho}d\rho + \int_{r}^{R}\frac{\ell^{1+\alpha}(\rho)}{\rho}d\rho\DIFaddbegin \DIFadd{.
}\DIFaddend \end{equation*}
%
...
\end{lemma}
\begin{proof}
Recall that $d(x):=\dist(x,\partial \Omega)$ and $d(x,y)=\min(d(x),d(y)).$ Then\DIFaddbegin \DIFadd{,
}\DIFaddend \begin{align*}
...
\rightarrow \tilde{A}_R(x):=\int_{B_R} \frac{\varphi_0(x)-\varphi_0(x+z)}{|z|} d z \quad \text { as } s \rightarrow 0^{+}\DIFaddbegin \DIFadd{.
}\DIFaddend \end{align*}
...
%
\DIFdelbeginFL %DIFDELCMD < \pgfplotstableset{
%DIFDELCMD < columns={N,h,L2norm,slopeL2,Hsnorm,slopeHs},
%DIFDELCMD < %%%
%DIFDELCMD < columns/N/.style={
%DIFDELCMD < column name=$N$,
%DIFDELCMD < },	
...
%DIFDELCMD < %%%
%DIFDELCMD < columns/L2norm/.style={
%DIFDELCMD < column name=$L^2$--norm,
%DIFDELCMD < sci,sci zerofill,sci subscript,
%DIFDELCMD < precision=3},
%DIFDELCMD < %%%
%DIFDELCMD < columns/slopeL2/.style={
...
%DIFDELCMD < column type/.add={}{|}},	
%DIFDELCMD < %%%
%DIFDELCMD < columns/Hsnorm/.style={
%DIFDELCMD < column name=$H^s$-norm,
%DIFDELCMD < sci,sci zerofill,sci subscript,
%DIFDELCMD < precision=3},
%DIFDELCMD < %%%
%DIFDELCMD < columns/slopeHs/.style={
%DIFDELCMD < column name=slope,
%DIFDELCMD < %sci,sci zerofill,sci subscript,
%DIFDELCMD < precision=2,
%DIFDELCMD < column type/.add={}{}},	
...
%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \pgfplotstableset{
