...
\section{Numerical evidence and illustrations}

In this section we present numerical approximations of some explicit solutions and we discuss some evidence for the optimality of the logarithmic convergence rate in Theorem \ref{main:thm:intro}. 

As mentioned before, one of the main difficulties to deal with problems associated with the logarithmic Laplacian is the lack of explicit solutions, in particular, no closed expression for the torsion function (i.e., taking  the right-hand side $f\equiv 1$) is known. Typically, such function is used to illustrate the optimality of the approximation rate of the numerical method (see, among others, \cite[Section 5]{AB17} or \cite[Section 5]{BHS19}) since it has the optimal regularity near the boundary. 

In view of this, we propose the following. First, consider the function $u:\R\to\R$ given by
\begin{align}\label{udef}
 u(x)=\frac{1}{\sqrt{-\ln\left(\frac{1-x^2}{2}\right)}} \chi_{(-1,1)}(x),
\end{align}
where $\chi_{(-1,1)}$ denotes the characteristic function in the interval $(-1,1)$. We justify this choice by noting that $u(x)$ behaves as $\ell^{1/2}(\delta(x))$ where $\delta(x)$ is the distance function to the boundary of the interval $(-1,1)$, decay which is conjectured to be the optimal one for problemas associated to $L_{\Delta}$, cf. \cite[Theorem 1.11]{CW19} and \cite{}. With the aid of Mathematica, we can compute directly $L_\Delta u(x)$ in $(-1,1)$, see Figure \ref{fig:1}. 
...
\begin{figure}[htbp]
	...
	legend style={cells={anchor=west},draw=none}]
	%	    
	\pgfplotstableread{./Programas/num_results/file50_lambda1.org}\solution      
...
Now, consider the interval $\Omega=(-1,1)$, let $h=\frac{1}{N+1}$ for $N\in\mathbb N$, and let $x_i:=-1+ih$ for $i\in\inter{0,N+1}$ so that $-1=x_0<x_1<\ldots<x_{N}<x_{N+1}=1$. Let $f_i:=L_\Delta u(x_i)$ for $i\in\inter{0,N+1}$, this is a good approximation of the coefficients $\int_{\R}L_\Delta u \phi_i\, dx$.  Let $F:=(f_i)$ and $\alpha:=A_h^{-1}F$, 
...
\end{align}
which is a good approximation for $u_h$, given in Theorem \ref{main:thm:intro}, see Figure \ref{fig:2}.
...
As seen in Figure \ref{fig:2}, the error is largest close to the boundary of the interval, where the function $u$ has a logarithmic behavior.  To quantify this error we compute the following quantities,
\begin{align*}
...
Note that these curves are not straight lines. This contrasts with the typical behavior of algorithms with convergence rate of order $h^p$ for some $p>0$; in particular, this is the case of the fractional Laplacian, see for instance \cite[Figure 8]{BH17} and \cite[Figure 1]{AB17}.  This suggests that the optimal convergence rate for the logarithmic Laplacian is \emph{not} of power-type, as shown in Theorem \ref{main:thm:intro}.

Although the curve in Figure \ref{l2_ah} looks almost straight, a computation of its slope (with respect to consecutive experiments) shows otherwise, as presented in Table \ref{table:1}.
...
%
\caption{Convergence data logarithmic Laplacian $L=1$.}
\label{table:1}
\end{table}
