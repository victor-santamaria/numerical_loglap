...
In this section we present numerical appoximations of some explicit solutions and we discuss some evidence for the optimality of the logarithmic convergence rate in Theorem \ref{main:thm:intro}. 

First, let $u:\R\to\R$ be given by
...
With Mathematica, we can compute $L_\Delta u$ in $(-1,1)$, see Figure \ref{fig:1}.
...
v_h(x):=\sum_{i=0}^{N+1}\alpha_i \phi_i, 
\end{align}
...
Although the curve in Figure \ref{fig:3} a) looks almost straight, a computation of its slope (with respect to consecutive experiments) shows otherwise, as presented in Table \ref{table:1}.
