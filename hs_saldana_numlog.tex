...
With Mathematica, we can compute $L_\Delta u$ in $(-1,1)$, see Figure \ref{fig:LogLap}.
...
v_N(x):=\sum_{i=0}^{N+1}\alpha_i \phi_i, 
\end{align}
which is a good approximation for $u_h$, given in Theorem \ref{main:thm:intro}, see Figure \ref{fig:1}.
...
Next we compute the following quantities,
\begin{align*}
 a_N&:=\int_{-1}^1 |u-v_N|^2\, dx\qquad \text{(the $L^2$-norm of the error between $u$ and $v$)},\\
 b_N&:=\int_{-1+h}^{1-h} |u-v_N|^2\, dx\qquad \text{(a local $L^2$-norm of the error between $u$ and $v$)},\\
 c_N&:=\sup_{(-1,1)} |u-v_N|\qquad \text{(the $L^\infty$-norm of the error between $u$ and $v$)}.
\end{align*}

We remark that close to 
...
%
\caption{Error data between $u$ and $v$. The $L^2-$norm column refers to $a_N$. }
\label{table:1}
\end{table}
