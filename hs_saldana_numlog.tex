...
\section{Numerical evidence and approximations}

In this section we present numerical appoximations of some explicit solutions. 

First, to illustrate Theorem \ref{main:thm:intro}, we use a function that exhibits numerically a logarithmic rate of convergence. To be more precise, let $u:\R\to\R$ be given by
\begin{align*}
 u(x)=\frac{1}{\sqrt{-\ln\left(\frac{1-x^2}{2}\right)}}.
\end{align*}

Consider the interval $\Omega=(-1,1)$, let $h=\frac{1}{N+1}$ for some $N\in\mathbb N$, and let $x_i:=-1+ih$ for $i\in\inter{0,N+1}$ so that $-1=x_0<x_1<\ldots<x_{N}<x_{N+1}=1$.  We computed with Mathematica 13.0 the values $f_i:=L_\Delta u(x_i)$ for $i\in\inter{0,N+1}$ and defined $F:=\sum$


reconstructed the exact solution with our algorithm.
...
\centering
%
\pgfplotstableread{./Programas/num_results/sol-log_convergence_data_21-06-2023_12h25.org}\data
%
\pgfplotstableset{
columns={N,h,L2norm,slopeL2,L2locnorm,slopeL2loc,Linfnorm,slopeinf},
%%%
...
%
\caption{Convergence data logarithmic Laplacian $L=3$.}
\label{table:2}
\end{table}
