...
In this section we present numerical approximations of some explicit solutions and we discuss some evidence for the optimality of the logarithmic convergence rate stated in Theorem \ref{main:thm:intro}. 
...
Because of this, we propose the following. Consider the function $u:\R\to\R$ given by
\begin{align}\label{udef}
 u(x)=\frac{1}{\sqrt{-\ln\left(\frac{1-x^2}{2}\right)}} \chi_{(-1,1)}(x),
\end{align}
where $\chi_{(-1,1)}$ denotes the characteristic function in the interval $(-1,1)$.

This choice is justified by the fact that a direct (nontrivial) calculation yields that $u$ has a uniformly bounded logarithmic Laplacian in $(-1,1)$ and it has the optimal boundary regularity for logarithmic Dirichlet problems, namely, it behaves as $\ell^{1/2}(d(x))$ (see the Introduction for the definition of $d$ and $\ell$). Here we only observe these facts numerically (see Figures \ref{fig:1} and \ref{fig:2}), but we will present the explicit calculation in a forthcoming paper. See also \cite[Theorem 1.11]{CW19} and \cite[Remark~1.5]{FJW22}.
