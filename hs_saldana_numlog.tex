...
\end{align*}
where ${\mathcal F}$ denotes the Fourier transform.  This operator appears naturally as a first order expansion of the (integral) fractional Laplacian $(-\Delta)^s$ as $s\to 0^+$; in particular, in \cite[Theorem 1.1]{CW19} it is shown that, for all $\varphi\in C^\infty_c(\R^N)$,
\begin{align*}
(-\Delta)^s\varphi = \varphi + sL_\Delta \varphi + o(s)\qquad \text{as $s\to 0^+$ in }L^p(\R) \text{ with }1<p\leq \infty,
\end{align*}
 where $(-\Delta)^s$ denotes the pseudodifferential operator with Fourier symbol $|\xi|^{2s}$.
...
To close this introduction, we discuss some numerical approximations of the torsion function and some explicit solutions...
