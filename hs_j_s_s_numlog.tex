...
Numerical evidence suggest that this type of rate cannot be improved. Moreover, we show that the stiffness matrix of logarithmic problems can be obtained as the derivative of the fractional stiffness matrix evaluated at $s=0$. Lastly, we investigate the relationship between the discrete eigenvalue problem and its convergence to the continuous one, and we also provide several numerical examples illustrating our results.
}
...
\end{equation}
 ...
 \Bc{We need to write a proper proof of Theorem \ref{main:thm:intro}.}
...
The paper is organized as follows. In Section~\ref{sec:localization-norm} we present the localization of the $\mathbb{H}$-norm. The main goal in this section is to provide the convergence of the method, see Theorem~\ref{density:thm} and some interpolation-type estimates. In Section~\ref{sec:error} we give the proof of Theorem~\ref{main:thm:intro}, see Theorem~\ref{thm:main-thm proof}. We calculate explicitly the stiffness matrix for the logarithmic Laplacian in Section~\ref{sec:stiffness} and we discuss the optimality of the convergence rate in Section~\ref{sec:numerics}. Finally, in Section~\ref{eigenvalue approximation:sec}, we give the proof of Proposition~\ref{eigenvalue approximation} and show that the eigenvalues of the stiffness matrix converge to eigenvalues of $L_{\Delta}$, see Proposition~\ref{prop:eigen-approx}.
