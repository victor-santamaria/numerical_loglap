...
\abstract{
We present the numerical analysis of a finite element method (FEM) for one-dimensional Dirichlet problems involving the logarithmic Laplacian, a pseudo-differential operator that appears as a first order expansion of the fractional Laplacian as the exponent $s\to 0^+$. Using recently obtained regularity results, we show rigorous error estimates and provide a logarithmic order of convergence in the energy norm by analyzing the problem in suitable \emph{log}-weighted spaces. We prove that the stiffness matrix of logarithmic problems can be obtained as the derivative of the fractional stiffness matrix evaluated at $s=0$. Furthermore, we investigate the relationship between the discrete eigenvalue problem and its convergence to the continuous one.  We also provide several numerical examples illustrating our results.
}
