...
\author{V\'ictor Hern\'andez-Santamar\'ia\footnote{The work of V. Hern\'andez-Santamar\'ia is supported by the program ``Estancias Posdoctorales por México para la Formación y Consolidación de las y los Investigadores por México'' of CONAHCYT (Mexico). He also received support from Projects A1-S-17475 and A1-S-10457 of CONAHCYT and by UNAM-DGAPA-PAPIIT grants IN109522, IN104922, and IA100324 (Mexico).} \and
 Sven Jarohs
 %\footnote{\blue{if needed add funding info}}
 \and
Alberto Salda\~{n}a\footnote{ A. Saldaña is supported  by  
CONAHCYT grant A1-S-10457 (Mexico) and by UNAM-DGAPA-PAPIIT grant IA100923 (Mexico).}\and
Leonard Sinsch
%\footnote{\blue{if needed add funding info}} 
}
...
\abstract{
\blue{In this work, we present the numerical analysis and implementation of a finite element approximation for one-dimensional elliptic problems involving the so-called logarithmic Laplacian, a pseudo-differential operator that appears as a first order expansion of the fractional Laplacian as $s\to 0^+$. We provide complete error estimates and provide an order of convergence of the method in the energy norm  by analyzing the problem in suitable \emph{log}-weighted spaces. Our approach, being 1D allows us to compute explicitly the associated stiffness matrix without using numerical integration as in other related fractional problems. As a further application of our results, we investigate some espectral properties of the logarithmic Laplacian, in particular, we show that the discrete eigenvalue problem converges to the continuous one. We provide several numerical examples illustrating our results. }
}
...
In this work, we focus our analysis to dimension one. This simplified setting allows us to present the key ideas and new concepts in a more transparent way and it has several other advantages; the most relevant one, is that in 1D we can provide an \emph{explicit formula} for the stiffness matrix.  This makes the implementation of the FEM very straightforward and fast in just a few lines of code.  \blue{We shall mention that this 1D approach has been successfully used for illustrating and identifying qualitative features of fractional elliptic and parabolic problems, see for instance, \cite{ABBM18,BdPM18,BHS19,CGH20} and we think that our work provides the community with a compelling tool to understand the particular pathologies of logarithmic problems and to test and deduce new conjectures for these difficult and elusive problems.} 

Furthermore, we believe that several of the ideas involved in the 1D analysis that we present can be generalized to dimensions 2 and 3, but certainly the higher-dimensional case is nontrivial and requires additional new ideas. \blue{Ver si vale la pena dejar esta frase así como lo descrito en la sección 9} See Section \ref{sec:o:p} for a discussion on the challenges in this direction and other open problems.
...
\mathcal A_{h}^{L}\alpha = F,
 ...
 \end{align*}
 see Lemma \ref{lem:derivative:s:m}.  This is possible, since a closed formula for the stiffness matrix $\mathcal A_{h}^{s}$ has been obtained in \cite{BH17}.  This can be seen as a discrete version of the expansion \eqref{intro:exp} in terms of the stiffness matrix. 
...
\SJ{Finally, to close the paper, we investigate the (Dirichlet) eigenvalues of the logarithmic Laplacian $L_{\Delta}$ in an interval $(-L,L)$ for some $L>0$. As shown in \cite[Theorem 1.4]{CW19}, the eigenvalues of $L_{\Delta}$ in a bounded domain are a sequence such that $\lambda_1<\lambda_2\leq \ldots\leq\lambda_n\to \infty$ for $n\to\infty$.  Moreover, by using scalings (see \cite[Lemma 2.5]{LW21}) it holds that if $\lambda_k(\Omega)$ denotes the $k$-th eigenvalue of $L_{\Delta}$ in a bounded $\Omega$, then $\lambda_k(R\Omega)=\lambda_k(\Omega)-2\ln(R)$ for $R>0$. In Section \ref{eigenvalue approximation:sec} we first show that the eigenvalues of the stiffness matrix defined in \eqref{s:m:intro} converge to the eigenvalues of the logarithmic Laplacian for $N\to \infty$, see Proposition \ref{prop:eigen-approx} below. Knowing from the scaling properties that, by sending $L$ to infinity, there are $L_i>0$ such that $\lambda_i=0$ in $(-L_i,L_i)$  for $i\in \N$, see Figure \ref{L vs condition}, it is enough to estimate the cases in which $0$ is an eigenvalue of $\mathcal{A}^L_h$. Using again the scaling properties, we may infer from this an approximation of the eigenvalues in a general interval. To be precise, we have
...
\end{proposition}
%
%To show here the simplicity of the eigenvalues, we use the simplicity of the (Dirichlet) eigenvalues of the fractional Laplacian $(-\Delta)^s$ in the interval $(-L,L)$, see \cite{add references}.
...
\SJ{After some introduction of our notation in Section \ref{sec:notation}, we present in Section \ref{sec:localization-norm} the tools on how to localize and discretize the $\mathbb{H}$-norm. The main goal in this section is to provide the convergence of the method, see Theorem \ref{density:thm}. In Section \ref{sec:estimates} we give several interpolation-type estimates and in Section \ref{sec:error} we complete the proof of Theorem \ref{main:thm:intro}, see Theorem \ref{thm:main-thm proof}. We complete this theoretical approach to the numerical approximation by calculating the stiffness matrix explicitly in Section \ref{sec:stiffness} and giving various numerical illustrations in Section \ref{sec:numerics}. Finally, in Section \ref{eigenvalue approximation:sec}, we give the proof of Proposition \ref{eigenvalue approximation} and show that the eigenvalues of the stiffness matrix converge to eigenvalues of $L_{\Delta}$, see Proposition \ref{prop:eigen-approx}. We close the paper with some open problems in Section \ref{sec:o:p}.
}
...
\leq \sum_{k\in I_i^\prime}|a_k|^2|\Omega_i|=h\sum_{k\in I_i^\prime}|a_k|^2.
\end{align*}
%
\end{proof}
...
\end{equation}
%
where $b_k:=\frac{1}{h}\int_{\Omega}(v-\overline v)\varphi_k dx$ \blue{and $I_i$ as defined in \eqref{def_Ii}}. Since $|\varphi_k|\leq 1$, 
...
\leq Ch^3+Ch\sum_{k\in I_i^\prime}a_k^2,\label{a2}
\end{align}
...
\end{equation}
%
where $b_k:=\frac{1}{h}\int_{\Omega}(v-\overline v)\varphi_k dx$ and \blue{$I_i$ is defined in \eqref{def_Ii}}. As we did in \eqref{abk}, for every $k\in I_i$,
\begin{align*}
    |b_k|^2\leq \frac{C}{h}\int_{T_{i}}|v-\overline{v}|^2\, dx,
\end{align*}
because the support of $\varphi_k$ is in $T_{i}$ for \blue{all $k\in I_i$}. Using \Cref{lem:poincare_type_weight}, we can estimate the right-hand side of the above expression and obtain
%
...
% %
% \begin{equation}
%     \sup_{v\in\mathcal V_h} \frac{|\cE_{L}(u_h,v)|}{\|v\|_{\mathbb{H}}}\geq \gamma_0\|u_h\|_{\mathbb H} \quad\text{ for all } u_h\in\mathcal V_h.
% \end{equation}
% %
...
%\\
%It remains to show the simplicity of the eigenvalues. For this, first note that the (Dirichlet) eigenvalues $\lambda_k^s$ of the fractional Laplacian in $(-L,L)$ are simple, \cite{add reference}. Denote the corresponding eigenfunctions as $\phi_k^s$ and we assume that $\phi_k^s$ is normalized. By \cite[Theorem 1.1]{FJW22} it holds $\phi_k^s\to \phi_k$ uniformly for $s\to 0^+$, where $\phi_k$ is an $L^2$-normalized eigenfunction of $L_{\Delta}$ in $(-L,L)$ corresponding $\lambda_k$, the $k$-th eigenvalue of $L_{\Delta}$ in $(-L,L)$. In the following, for $k\in \N$ denote by $m_k$ the multiplicity of $\lambda_k$ and we write $\phi_{ki}$, $i=1,\ldots,m_k$ for the eigenfunctions forming the orthonormal basis of the eigenspace corresponding to $\lambda_{k}$. We may put $\phi_k=\phi_{k1}$. Assume there is $k\in \N$ with $m_k>1$. Since $\{\phi_{i}^s\}_{i\in \N}$ forms an orthonormal basis of $L^2((-L,L))$, there are values $\alpha_i(s)\in \R$ such that
%$$
%\phi_{k,2}=\sum_{i\in \N} \alpha_i(s)\phi_i^s\quad\text{for $s\in(0,1)$.}
%$$
%Since $\phi_{k,2}$ is orthogonal to $\phi_{i}$ for $i\in \N$, it follows that for $i\in \N$ we have
%$$
%\lim_{s\to 0^+}\alpha_i(s)=\lim_{s\to 0^+}\int_{(-L,L)}\phi_{k,2}\phi_i^s\,dx=\int_{(-L,L)}\phi_{k,2}\phi_i\,dx=0.
%$$
\end{proof}
}
...
\SJ{We close this section with the explicit numerical approximations. In Figure \ref{L vs condition} the condition of $\mathcal{A}^L_h$ is plotted for a fixed $N$ and varying $L$. The singularities then indicated the relevant areas in which $L$ can be chosen to have $0$ as an eigenvalue in $(-L,L)$. To approximate the exact values, using the \textit{``Golden-section search''} in certain intervals, we find $L_i$ for $i=1,\ldots,6$ as follows.
