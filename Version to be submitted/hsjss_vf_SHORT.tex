...
We close this section with explicit numerical approximations. In \Cref{Fig:eigen}, we show the numerical approximation of the first and second eigenfunctions of \( L_{\Delta} \) for two different intervals. As noted in \cite[Theorem 1.4]{CW19} (see also \cite{FJW22}), the first eigenfunction is unique and can be chosen to be positive, which agrees with the experiment. On the other hand, very few properties are known for the second eigenfunction. From the figure, we observe that it also scales well across the intervals and changes sign, suggesting an interesting direction for further research.
...
Lastly, \Cref{Fig:blow} illustrates the behavior of the torsion function as \( L \to L_1 \). Since this corresponds to an eigenvalue approaching zero, we expect the function to blow up in this limit, which is indeed observed in the figure. Moreover, as \( L \to L_1^- \), the solution starts changing sign, reflecting the fact that the first eigenvalue becomes negative.
...
\begin{figure}[htb]
	...
	\begin{axis}[xlabel={$x$},xmin=-0.699,xmax=0.699,legend pos=outer north east, ymin=0,
	legend plot pos=left, xtick={-0.699,0,0.699}, xticklabels={$-L$,0,$L$},
	legend style={cells={anchor=west},draw=none}]
	%	    
	\pgfplotstableread{sol-log_L_0699.org}\solution      
	...
	\subfloat[$L=0.7090$]{
	\begin{tikzpicture}[scale=0.4]
	\begin{axis}[xlabel={$x$},xmin=-0.709,xmax=0.709,legend pos=outer north east,
	legend plot pos=left, xtick={-0.709,0,0.709}, xticklabels={$-L$,0,$L$}, ymax=0,
	legend style={cells={anchor=west},draw=none}]
	%	    
	\pgfplotstableread{sol_log_L_0709.org}\solution      
	...
	legend plot pos=left, xtick={-0.719,0,0.719}, xticklabels={$-L$,0,$L$}, ymax=0,
	legend style={cells={anchor=west},draw=none}]
	%	    
	\pgfplotstableread{sol-log_L_0719.org}\solution      
...
%	\end{tikzpicture}
%	} 
	\caption{Behavior of the torsion function for different intervals $(-L,L)$. In these experiments $N=2^{10}$. For $L=0.709$, the (numerical) first eigenvalue is $\lambda_1\approx -6.9497\times 10^{5}$, which agrees with the results of \Cref{eigenvalue approximation}.
}
	\label{Fig:blow}
\end{figure}
