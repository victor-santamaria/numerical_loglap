...
For any $h<1/2$ and any $v\in C_c^\infty(\Omega)$,
%
\begin{equation}\label{eq:rate_H_c_infty}
    \|v-I_h v\|^2_{\mathbb H(\Omega)}\leq C(|v|_{\infty},|v^\prime|_{\infty},L)\frac{h}{\ell(h)},
\end{equation}
%
...
\begin{proof}
Along the proof, $C>0$ denotes possibly different constants that depend at most on $L$, $|v|_{\infty}$, and $|v^\prime|_{\infty}$.  Combining \eqref{eq:approxim_estimate} and \Cref{rem:l2_stability} with \Cref{lem:localization_enorm} we obtain
%
\begin{align*}
...
%
\begin{align*}
    \|v-I_h v\|^2_{\mathbb H(\Omega)} &\leq Ch^2+Ch^2\ell^{-1}(h)+C\left(\|v\|_{L^\infty(\Omega)}^2+\|I_hv\|^2_{L^\infty(\Omega)}\right)\sum_{i\in\{0,N\}}\int_{\Omega_i}\ell^{-1}(d(x))\,dx\leq C \frac{h}{\ell(h)},
\end{align*}
as claimed.
\end{proof}
...
\begin{align*}
    ...
    &\leq \frac{2\epsilon}{3}+\|w-I_hw\|_{\mathbb H(\Omega)} \leq \frac{2\epsilon}{3}+{C(|w|_{\infty},|w^\prime|_{\infty},L)}h/\ell(h),
\end{align*}
%
where we have used that $\|\Pi_h\|_*=1$, property \eqref{eq:prop_infimo}, and \Cref{eq:estab_H_c_infty}. {Finally, we choose $h>0$ small enough so that ${C(|w|_{\infty},|w^\prime|_{\infty},L)}h/\ell(h)\leq \frac{\epsilon}{3}$ (note that $w\in C^\infty_c(\Omega)$ is independent of $h$)}. This yields the claim.
\end{proof}
