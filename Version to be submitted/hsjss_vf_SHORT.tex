...
\begin{equation}\label{eq:rate_H_c_infty}
    \|v-I_h v\|^2_{\mathbb H(\Omega)}\leq C(|v|_{\infty},|v^\prime|_{\infty},L)h\,\ell^{-1}(h),
\end{equation}
%
...
\begin{align*}
    ...
    &\leq \frac{2\epsilon}{3}+\|w-I_hw\|_{\mathbb H(\Omega)} \leq \frac{2\epsilon}{3}+{C(|w|_{\infty},|w^\prime|_{\infty},L)}h\, \ell^{-1}(h),
\end{align*}
%
where we have used that $\|\Pi_h\|_*=1$, property \eqref{eq:prop_infimo}, and \Cref{eq:estab_H_c_infty}. {Finally, we choose $h>0$ small enough so that ${C(|w|_{\infty},|w^\prime|_{\infty},L)}h\, \ell^{-1}(h)\leq \frac{\epsilon}{3}$ (note that $w\in C^\infty_c(\Omega)$ is independent of $h$)}. This yields the claim.
\end{proof}
